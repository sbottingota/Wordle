...
public final class WordleMain {
	...
	private static final String GREEN = "\u001B[42m\u001b[30m";
	private static final String YELLOW = "\u001B[43m\u001b[30m";
	private static final String WHITE = "\u001B[47m\u001b[30m";
	private static final String RESET = "\u001B[0m";

	private static final char[][] keys = { "QWERTYUIOP".toCharArray(), "ASDFGHJKL".toCharArray(),
			"ZXCVBNM".toCharArray() };
	...
	// sets the ansiColor of a particular key on the keyboard (not guessed, not in
	// the word, not in the right place, in the right place)
	...
	private static void changeKeyboardColors(String guess, String wordle) {
		...
		for (int character = 0; character < guessArray.length; character++) {
			if (guessArray[character] == wordleArray[character]) {
				setKeyboardLetterColor(wordleArray[character], GREEN);
				//guessArray[character] = ' ';
				
			} else if (Arrays.asList(charArrayToCharObjArray(wordleArray)).contains(
					Character.valueOf(guessArray[character])) && !getKeyColor(guessArray[character]).equals(GREEN)) {
